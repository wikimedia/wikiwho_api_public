...
urlpatterns = [
    ...
    url(r'^content/(?P<article_name>.+)/(?P<revision_id>[0-9]+)/$',
        WikiwhoApiView.as_view(actions={'get': 'get_article_revision'}), name='content_article_name_rev_id'),
    url(r'^content/(?P<article_name>.+)/$',
        WikiwhoApiView.as_view(actions={'get': 'get_article_by_name'}), name='content_article_name'),
